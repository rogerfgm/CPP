...
// エンジン設定ファイルのパス
#ifdef _WIN32
const char* iniFilePath = ".\\Lesserkai.ini";
#else
const char* iniFilePath = "./Lesserkai.ini";
#endif
...
int main()
{
	...
	::GetPrivateProfileString("CustomSettings", "BookFile", "public.bin", iniValueBuf, 256, iniFilePath);
	string bookFileStr = iniValueBuf;
	::GetPrivateProfileString("CustomSettings", "UseBook", "true", iniValueBuf, 256, iniFilePath);
	string useBookStr = iniValueBuf;
	bool isUseJoseki = useBookStr == "true";
	...
	while (true) {
		while (true) {
			...
			if (strncmp(buf, "usi", len) == 0) {
				...
				printf("id name Lesserkai 1.3.4\n");
				...
				if (strncmp(buf, "setoption name USI_Ponder value true", len) == 0) {
					...
					// 初期設定ファイルに保存しておく
					::WritePrivateProfileString("CustomSettings", "BookFile", bookFileStr.c_str(), iniFilePath);
				} else if (strncmp(buf, "setoption name UseBook value ", strlen("setoption name UseBook value ")) == 0) {
					useBookStr = buf + strlen("setoption name UseBook value ");
					isUseJoseki = useBookStr == "true";
					// 初期設定ファイルに保存しておく
					::WritePrivateProfileString("CustomSettings", "UseBook", isUseJoseki ? "true" : "false", iniFilePath);
				}
